#include "point_cloud_heat_solver_ext.h"


namespace geometrycentral {
namespace pointcloud {

PointCloudHeatSolverExt::PointCloudHeatSolverExt(PointCloud& cloud_, PointPositionGeometryExt& geom_, 
                                                    SparseMatrix<double>& L_, 
                                                    SparseMatrix<double>& M_, 
                                                    std::vector<std::vector<size_t>>& neighbors,
                                                    double tCoef_)
    : tCoef(tCoef_), cloud(cloud_), geom(geom_), L(L_), M(M_) {
  ...
  geom.reset_neighborhoods(neighbors);
...
void PointCloudHeatSolverExt::ensureHaveHeatDistanceWorker() {
  if (heatDistanceWorker != nullptr) return;

  heatDistanceWorker.reset(new surface::HeatMethodDistanceSolverExt(*geom.tuftedGeom, L, M, tCoef));
}

void PointCloudHeatSolverExt::ensureHaveVectorHeatSolver() {
  ...
  heatDistanceWorker.reset(
      new surface::HeatMethodDistanceSolverExt(*geom.tuftedGeom, L, M, tCoef, false)); // we already have the tufted IDT
...
PointData<double> PointCloudHeatSolverExt::computeDistance(const Point& sourcePoint) {
  std::vector<Point> v{sourcePoint};
  return computeDistance(v);
}
PointData<double> PointCloudHeatSolverExt::computeDistance(const std::vector<Point>& sourcePoints) {
...
PointData<double> PointCloudHeatSolverExt::extendScalars(const std::vector<std::tuple<Point, double>>& sources) {
  ensureHaveHeatDistanceWorker();
...
PointData<Vector2> PointCloudHeatSolverExt::transportTangentVector(const Point& sourcePoint, const Vector2& sourceVector) {
...
PointData<Vector2>
PointCloudHeatSolverExt::transportTangentVectors(const std::vector<std::tuple<Point, Vector2>>& sources) {
...
PointData<Vector2> PointCloudHeatSolverExt::computeLogMap(const Point& sourcePoint) {
  ensureHaveHeatDistanceWorker();
  ensureHaveVectorHeatSolver();
