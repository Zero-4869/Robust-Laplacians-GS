...
class PointCloudHeatSolverExt {

public:
  // === Constructor
  PointCloudHeatSolverExt(PointCloud& cloud, PointPositionGeometryExt& geom, SparseMatrix<double>& L, 
                          SparseMatrix<double>& M, std::vector<std::vector<size_t>>& neighbors, double tCoef = 1.0);
  PointCloudHeatSolverExt(PointCloud& cloud, PointPositionGeometryExt& geom, SparseMatrix<double>& L, 
                          SparseMatrix<double>& M, double tCoef = 1.0);
...
private:
  ...
  std::unique_ptr<surface::HeatMethodDistanceSolverExt> heatDistanceWorker;
