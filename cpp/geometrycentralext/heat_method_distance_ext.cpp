#include "heat_method_distance_ext.h"
...
VertexData<double> heatMethodDistanceExt(IntrinsicGeometryInterface& geom, const SparseMatrix<double>& L, 
                                                     const SparseMatrix<double>& M, Vertex v) {
  return HeatMethodDistanceSolverExt(geom, L, M).computeDistance(v);
}

HeatMethodDistanceSolverExt::HeatMethodDistanceSolverExt(IntrinsicGeometryInterface& geom_, 
                                                     const SparseMatrix<double>& L, 
                                                     const SparseMatrix<double>& M_,
                                                    double tCoef_,
                                                   bool useRobustLaplacian_)
    : tCoef(tCoef_), useRobustLaplacian(useRobustLaplacian_), M(M_), mesh(geom_.mesh), geom(geom_) {
...
SurfaceMesh& HeatMethodDistanceSolverExt::getMesh() { return useRobustLaplacian ? *tuftedMesh : mesh; }
IntrinsicGeometryInterface& HeatMethodDistanceSolverExt::getGeom() {
  return useRobustLaplacian ? *tuftedIntrinsicGeom : geom;
}

VertexData<double> HeatMethodDistanceSolverExt::computeDistance(const Vertex& sourceVert) {
  // call general version
  return computeDistance({SurfacePoint(sourceVert)});
}
VertexData<double> HeatMethodDistanceSolverExt::computeDistance(const std::vector<Vertex>& sourceVerts) {
...
VertexData<double> HeatMethodDistanceSolverExt::computeDistance(const SurfacePoint& sourcePoint) {
  // call general version
  return computeDistance(std::vector<SurfacePoint>{sourcePoint});
}

VertexData<double> HeatMethodDistanceSolverExt::computeDistance(const std::vector<SurfacePoint>& sourcePoints) {
...
Vector<double> HeatMethodDistanceSolverExt::computeDistanceRHS(const Vector<double>& rhsVec) {
  getGeom().requireHalfedgeCotanWeights();
  getGeom().requireHalfedgeVectorsInFace();
  getGeom().requireEdgeLengths();
  getGeom().requireVertexIndices();
  ...
  // for(size_t iPt=0;iPt<divergenceVec.size();iPt++){
  //   divergenceVec[iPt] *= M.coeff(iPt, iPt);
  // }
